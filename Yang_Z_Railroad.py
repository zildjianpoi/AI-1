...
def a_star(start, goal, graph, tri, heuristic=dist_heuristic):
   ...
   if start == goal:
      return []
   frontier = []
   s = graph[1][start]     #makes start and goal cities into city codes
   g = graph[1][goal]
   if s == g: return []
   ###
   ex = dict()
   h = heuristic(s, g, graph)          #Heuristic really represents frontier[i][0] AKA the total FUNCTION COST (h+p)
   frontier.append((h, s, [s], 0))
   i = 0 #index counter
   # Your code goes here
   while len(frontier)>i:        #loop until frontier index surpasses range
       if frontier[i][0] > h:    #Only sort and remove when new total function cost more than old one 
           frontier[:i] = []
           frontier = sorted(frontier)
           #print("save time??")
           i = 0
       curr = frontier[i][1]        #split tuple into individal values
       n += 1
       #f = frontier.pop(0)
       #curr = f[1]
       h = frontier[i][0]
       ...
       if curr in ex and ex[curr] < pathcost:  #if curr already explored and has a larger cost, ignore
           continue
       ex[curr] = pathcost             #update lowest pathcost to node (since heuristic always constant, not a factor)
       #ex.add(curr)
       for c in graph[2][curr]:        # For all Children(for loop):
           #print("Child: " + c)
           cost = graph[2][curr][c]       #stepcost
           if c == g:                     #if curr = goal, return and print statistics
               print("Nodes: " + str(n))
               print("pathcost: " + str(pathcost+cost))
               if tri:                    # Tri = if used in Tri-Directional Search
                  return (p+[c], n, pathcost+cost)    #must return total nodes explored and pathcost in addition to other part of path if in Tri-Search
               return p + [c]
           if c in ex and ex[c] < pathcost + cost:    #if child already explored and higher cost, ignore
               continue
           frontier.append((heuristic(c, g, graph)+ pathcost + cost, c, p + [c], pathcost+cost)) #append child to frontier
   return None
   #return []
...
def bidirectional_BFS(start, goal, graph):
   ...
   q = [[s]]
   q2 = [[g]]
   i = 0                         #Gabor index non removal count
   ii = 0
   while len(q) > i:
      curr = q[i][-1]
      path1 = q[i]
      n += 1
      if i == 100:              #Rather than removing every time, remove every 100 to save time
         q[:100] = []
         i = 0
      i += 1
      if curr in ex2:         #Checks if curr node is in other explored: if it is, return the path
         #while q2[ii][-1] != curr:
            #ii += 1
         ex2[curr].reverse()
         #print(path2)
      ...
      if ii == 100:
         ...
         ii = 0
      ii += 1
      #No need for a second check as BFS will always go though every layer (end up with same ans)(Not so for A*)
      """if curr in ex2:      
         while q2[ii][-1] != curr:
            ii += 1
         path2 = q2[ii].reverse()
...
def bidirectional_a_star(start, goal, graph, heuristic=dist_heuristic):
   ...
   """
   Because bi A* does not guarentee least path on the first path found, I do not immediately return. 
   This causes this search to take much longer than the other searches. 
   For reference, St Louis to Fort Worth takes around 20 seconds BUT the opposite takes around 3 seconds.
   """
   minpath = None             #stores min path: does not immediatly return, checks for better path
   limit = 0                  #stores min path's cost
   n = 0                #Nodes expanded count
   if start == goal:
      return []
   ex = dict()             #2 Exploreds
   ex2 = dict()            #node: (cost, path)
   s = graph[1][start]     #Translates city names to city codes
   g = graph[1][goal]
   h = heuristic(s, g, graph) #Original heuristical distance between start and goal
   h2 = h         #same thing
   q = [(h, [s], s, 0)]       #frontier for start->goal
   q2 = [(h2, [g], g, 0)]       #frontier for goal->start
   i = 0                   #index counters for gabor's method of saving time
   ii = 0 
   while len(q) > i and len(q2) > ii:       #while the index counter doesn't go out of the frontier's range
      if q[i][0] > h:                     #if curr total cost(h + p) is bigger than prev, Resort frontier
         q[:i] = []                          #remove already explored indexes
         q = sorted(q)
         i = 0                            #reset index count
      #Separating the tuple from frontier into:
      curr = q[i][2]   #current node                   
      path1 = q[i][1]   #current path
      h = q[i][0]       #current heuristic
      pathcost = q[i][-1]  #current pathcost
      n += 1      #increment expanded nodes
      i += 1         #increment index
      if curr in ex and ex[curr][0] < pathcost:    #if curr node in explored and has higher cost, ignore it
         continue
      ex[curr] = (pathcost, path1)           #else update cost and path
      for c in graph[2][curr]:            #FOR EACH CHILD:
         cost = graph[2][curr][c]         #find stepcost
         if (c in ex and ex[c][0] < pathcost + cost) or (limit!=0 and pathcost > limit*2/3): #if child in explored and costs more than prev or went over the minpath limit, ignore it
               continue
         if curr in ex2:                           # if the curr node is in the explored of the other A*
            ex2[curr][-1].reverse()                # reverse the end->start side
            path1 = path1 + ex2[curr][-1][1:]      #add the 2 parts of the path(getting rid of the repeat node)
            totCost = pC(path1, graph)             #total cost of this concoted path
            if minpath == None or totCost < limit: #Updates minimum Cost Path and that Cost if the cost is less than before
               minpath = path1
               limit = totCost
         q.append((heuristic(c, g, graph) + pathcost + cost, path1+[c], c, pathcost+cost))      #appends new child to frontier
         
      """
      Same thing as above, just with everything as version two instead of v1 (like ex2 instead of ex)
      """
      ...
      for c in graph[2][curr]:
         ...
         if c in ex2 and ex2[c][0] < pathcost + cost:
               continue
         if curr in ex:                         # if the curr node is in the explored of the other A*
            path2.reverse()
            path2 = ex[curr][-1] + path2[1:]
            totCost = pC(path2, graph)
            if minpath == None or totCost < limit: #Updates minimum Cost Path and that Cost if the cost is less than before
               minpath = path2
               limit = totCost
...
def tridirectional_search(goals, graph, heuristic=0):         
   ...
   side01 = dist_heuristic(goals[0],goals[1],graph)      #calculate all 3 predicted side lengths
   side02 = dist_heuristic(goals[0],goals[2],graph)
   side12 = dist_heuristic(goals[2],goals[1],graph)
   m = max(side01, side02, side12)                    #find the longest side length -> use the other 2 shorter side lengths to find path
   #print(goals)
   goals = [graph[1][c] for c in goals]
   #print(goals)
   if side01 == m:         #if a side-ab is the max side, then c, the one in neither, must be in the middle
      s = goals[0]         #set start goal and middle based on that
      e = goals[1]
      middle = goals[2]
   elif side02 == m:
      middle = goals[1]
   ...
   #print((s, middle, e))
   #print(s,middle,e)
   pathpt1 = a_star(s, middle, graph, True)     #Use A-star for both start -> middle and middle -> end
   pathpt2 = a_star(middle, e, graph, True)     #returns a tuple (path, nodes explored, pathcost)
   path = pathpt1[0][:-1] + pathpt2[0]    #add the two paths together, getting rid of the repeat node
   print("Nodes Tri-Dir: " + str(pathpt1[1] + pathpt2[1]))   #add nodes ex to print all nodes ex
   print("Total Pathcost: ", pathpt1[2] + pathpt2[2])       #add pathcosts to calc pathcost
   print("City Path: ", cities(path, graph))          #city path method to find cities in path
   return path
   
def cities(ll, graph):     #go through all nodes in path to find cites and make new list of major cities visited
   city = []
   for c in ll:
      if c in graph[1]:
...
def pathCost(ll, graph):         #calculates and prints both the pathcost and the city path in one loop iteration to save time
   pc = 0
   city = []
...
def pC(ll, graph):            #only calculates to pathcosts and returns it incase we do not want to print it yet
   pc = 0
   for i in range(len(ll)-1):
      pc += calc_edge_cost(ll[i], ll[i+1], graph[0])
   return pc
...
def main():
   ...
   print ("\nA* Search Summary")
   cur_time = time.time()
   a_star_path = a_star(start, goal, graph, False)
   next_time = time.time()
   ...
   cur_time = time.time()
   bi_a_path = bidirectional_a_star(start, goal, graph)#[0]
   #test = bidirectional_a_star("St Louis", "Fort Worth", graph, False)
   next_time = time.time()
